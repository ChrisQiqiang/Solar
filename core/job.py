...
class Task(object):
    ...
    def __str__(self):
        return str(self.job.id) + " " + str(self.arrived_timestamp) + " " + str(self.started_timestamp)  \
               + " " + str(self.finished_timestamp)
...
class TaskInstance(object):
    def __init__(self, env, task, task_instance_index, task_instance_config):
        ...
        self.duration = max(task_instance_config.duration, 0)
        self.arrived_timestamp = None
        self.machine = None
        self.process = None
        self.new = True
